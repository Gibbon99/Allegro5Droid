...
ALLEGRO_TIMER *aStarPathTimer;
int           aStarPathInterval;      // From script
...
//-----------------------------------------------------------------------------
{
	...
	int whichTile = shipLevel.at (path[whichPath].whichLevel).tiles[tileIndex];
...
//	USE_VALID_TEST
	...
	if (true == gam_AStarIsTileSolid ((tempNode.tileLocation.y * (shipLevel.at (path[whichPath].whichLevel).levelDimensions.x) + tempNode.tileLocation.x), whichPath))
	{
...
//------------------------------------------------------------
{
	...
	al_draw_line(lineStart.x, lineStart.y, lineFinish.x, lineFinish.y, al_map_rgba(255, 0, 0, 255), 2);

	al_draw_filled_rounded_rectangle ( lineStart.x - 8, lineStart.y - 8, lineStart.x + 8, lineStart.y + 8, 2, 2, al_map_rgb ( 0, 0, 50 * count ) );

	al_draw_filled_rounded_rectangle ( lineFinish.x - 4, lineFinish.y - 4, lineFinish.x + 4, lineFinish.y + 4, 2, 2, al_map_rgb ( 0, 0, 0 ) );
}
...
//------------------------------------------------------------
{
	log_logMessage (LOG_LEVEL_INFO, sys_getString ("Number of paths [ %i ]", path.size ()));

	for (int i = 0; i != (int) path.size (); i++)
	{
		log_logMessage (LOG_LEVEL_INFO, sys_getString ("inUse [ %s ] Droid [ %i ] Path [ %i ] - Waypoints Ready [ %s ] - Size foundNodes [ %i ] ",
				path[i].inUse ? "true" : "false", path[i].whichDroid, i, path[i].wayPointsReady ? "true" : "false", path[i].foundPath.size ()));
	}
}
...
//------------------------------------------------------------
{
	...
	shipLevel.at (path[whichPath].whichLevel).droid[path[whichPath].whichDroid].aStarPathIndex = -1;
...
//	USE_VALID_TEST
	...
	{
		log_logMessage (LOG_LEVEL_INFO, sys_getString ("ERROR: No open nodes to search in gam_AStarFindLowestCostNode - path [ %i ]", whichPath));
		return -1;
	}
...
void gam_AStarProcessPaths (const std::string levelName)
//-------------------------------------------------------------------
{
	for (int i = 0; i != (int) path.size (); i++)
	{
...
{
#ifdef DEBUG_ASTAR
	log_logMessage(LOG_LEVEL_DEBUG, sys_getString("gam_AStarStartPathThread -> Path ID [ %i ] Droid [ %i ]", whichPath, path[whichPath].whichDroid ));
#endif
}
...
int gam_aStarPathTimerCallback (int interval, void *param)
// ----------------------------------------------------------------------------
{
	if (!doaStarPath)
		return interval;
