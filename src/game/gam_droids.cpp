							...
							switch (shipLevel.at (levelName).droid[index].currentMode)
								{
									case DROID_MODE_NORMAL:
										...
										io_renderTintedSpriteFrame (shipLevel.at (levelName).droid[index].spriteName,
												io_getFrame (), drawPosition.x, drawPosition.y, 0, 0, 0);
...
//		tempDroid.isAlive = true;
			...
			tempDroid.worldPos = shipLevel.at ( levelName ).wayPoints[tempDroid.wayPointIndex];
				...
				if ( -1 == sourceDroid ) // Player bullet
					{
						...
						if ( shipLevel.at ( lvl_returnLevelNameFromDeck ( whichLevel )).droid[whichDroid].currentMode == DROID_MODE_EXPLODING )
							return;
