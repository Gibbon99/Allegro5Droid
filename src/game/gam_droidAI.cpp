	...
	switch (tileType)
		{
			...
			if (shipLevel.at (levelName).droid[whichDroid].worldPos.x <= playerDroid.worldPos.x)    // Flee to the left
				{
					for (int i = 0; i != (int) shipLevel.at (levelName).levelDimensions.x *
					                     (int) shipLevel.at (levelName).levelDimensions.y; i++)
						{
							destX++;
			...
			else    // Flee to the right
				{
					...
					for (int i = (int) (shipLevel.at (levelName).levelDimensions.x - 5) *
					             (int) (shipLevel.at (levelName).levelDimensions.y - 5); i > 0; i--)
						{
							destX--;
							if (destX == (0))
								{
