...
//-----------------------------------------------------------------------------
{
	...
	switch (shipLevel.at(levelName).droid[whichDroid].ai_moveMode)
	{
		case AI_MODE_WAYPOINT:
			...
			{
				case WAYPOINT_UP:
					if (shipLevel.at(levelName).droid[whichDroid].wayPointIndex <
					    shipLevel.at(levelName).numWaypoints - 1)
					{
	...
	else
	{
		...
		switch (currentAlertLevel)
		{
			case ALERT_GREEN_TILE:
				shipLevel.at(levelName).droid[whichDroid].currentSpeed += playerAcceleration; //dataBaseEntry[shipLevel.at (levelName).droid[whichDroid].droidType].accelerate;
				break;

			case ALERT_YELLOW_TILE:
				shipLevel.at(levelName).droid[whichDroid].currentSpeed += (
						dataBaseEntry[shipLevel.at(levelName).droid[whichDroid].droidType].accelerate * 1.2f);
				break;

			case ALERT_RED_TILE:
				shipLevel.at(levelName).droid[whichDroid].currentSpeed += (
						dataBaseEntry[shipLevel.at(levelName).droid[whichDroid].droidType].accelerate * 1.5f);
				break;
		}

		if (shipLevel.at(levelName).droid[whichDroid].currentSpeed > playerMaxSpeed) //[shipLevel.at (levelName).droid[whichDroid].droidType].maxSpeed )
		{
			shipLevel.at(levelName).droid[whichDroid].currentSpeed = playerMaxSpeed; //[shipLevel.at (levelName).droid[whichDroid].droidType].maxSpeed;
		}
	}
...
//-----------------------------------------------------------------------------
{
	if (shipLevel.at(levelName).droid[whichDroid].droidType <
	    shipLevel.at(levelName).droid[shipLevel.at(levelName).droid[whichDroid].collidedWith].droidType)
	{
		switch (shipLevel.at(levelName).droid[whichDroid].wayPointDirection)
		{
			case WAYPOINT_DOWN:
